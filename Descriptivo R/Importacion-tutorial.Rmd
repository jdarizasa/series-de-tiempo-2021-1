...
sercamrelint=ts(camrelintanual,start=c(1988,01),frequency=12) #ts para objeto time series
sercamrelint
plot(sercamrelint,main="Cambios relativos") #son la misma grafica
plot.ts(sercamrelint)
acf(sercamrelint,ci.type='ma')
acf(sercamrelint,type='partial')
str(sercamrelint)
head(time(sercamrelint),20)
head(cycle(sercamrelint),15) #duracion del ciclo de tiempo
frequency(sercamrelint)
deltat(sercamrelint)###en este caso es 1/12
ts_info(sercamrelint) #descripcion de la serie de tiempo
```

```{r gráfico interactivo}
TSstudio::ts_plot(sercamrelint,title="",slider=TRUE) #grafico interactivo
```


```{r grafico interactivo}
library(dygraphs)
dygraph(sercamrelint,main="Gráfico de Retorno Interés Anual", ylab="Cambios relativos del tipo de interés anual")%>% dyRangeSelector() #grafico interactivo por %>%
```
...
ts_info(EURO_Brent) #viene en formato mes - año
...
index(EURO_Brent) <- as.Date(index(EURO_Brent)) #lo cambiamos a fecha (dia, mes, año)
head(EURO_Brent)
class(index(EURO_Brent))
...
###establecemos cual es el índice para que no sea el yearmon
...
US_indicators_xts=xts(x = US_indicators[,c("Vehicle Sales",
   "Unemployment Rate")],frequency = 12, order.by=US_indicators$Date) # hacemos la transformacion, especificando el timestamp y los datos.
head(US_indicators_xts)
ts_info(US_indicators_xts)
class(US_indicators_xts)
frequency(US_indicators_xts) # no deberia ser frequencia 1 xd
...
tformat(Michigan_CS) <- "%m-%d-%Y" #transformamos el yearmon a date
head(Michigan_CS)
```
...
```{r Colcap}
Colcap<- read_excel("Datos historicos COLCAP-3.xlsx")
...
str(dates_df)
###Convierte directamente de caractér a Fecha, note que el objeto caracter tiene la misma estructuta ISO8601, entonces no se necesita especificar el formato.
dates_df$Japanese_format_new <- as.Date(dates_df$Japanese_format) #creamos la nueva variable como fecha
...
head(dates_df$US_format) #caracter
dates_df$US_format_new <- as.Date(dates_df$US_format, format = "%m/%d/%Y") #fecha
head(dates_df$US_format_new)
###Para entender el formato, ver páginas 45 y 51.
###Note otro ejemplo
head(dates_df$US_long_format)
dates_df$US_long_format_new <- as.Date(dates_df$US_long_format, format ="%A, %B %d, %Y") #primero el dia, mes, numero de dia y año pero es todo fecha
...
hourly_index <- seq.POSIXt(from = as.POSIXct("2018-06-01"), by = "hours",
   length.out = 48) #arranca un dia, separado por horas y que salgan 48 observaciones
str(hourly_index)
head(hourly_index)
tail(hourly_index)
```
