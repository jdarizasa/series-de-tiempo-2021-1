...
library(TSstudio)
data(USUnRate)
ts_info(USUnRate) #descripcion, recuerde
#frecuencia es cuantas observaciones por año
...
* Tendencia? Parece que sí porque los valores medios cambian por decada.
* Estacionalidad? No es seguro.
* Cíclos? Parece que si, por los valles y picos que tiene.
* Varianza marginal no constante? No es muy claro la heterocedasticidad.
Vamos a seleccionar un periodo de tiempo mas corto

```{r DesempleoUS selec periodo}
unemployment <- window(USUnRate, start = c(1990,1)) #de la serie, vamos a cambiar el punto de inicio (1990 ahora). Sirve cuando el resto de info no me interesa
   ts_plot(unemployment,
           title = "US Monthly Unemployment Rate",
           Ytitle = "Unemployment Rate (%)",
           Xtitle = "Year",
           Xgrid = TRUE,
Ygrid = TRUE) #no usa plot, sino ts_plot para ser interactivo
```
Note que aquí podemos ver varias características: Estacionalidad(no es tan evidente), tres periodos de cíclos, el primero de 1990 a 2000,el segundo de 2000 a 2007, y el tercero de 2007 a 2019 con amplitud más grande. No parece tener una heterocedasticidad marginal.
...
DesempleoyEmpleo_1=DesempleoyEmpleo %>% map_df(rev) #el operador %>% reversa la base de datos, para que este en el correcto orden cronologico
...
order.by = Fechas) #xts means extensible, fechas con yearmon
ts_info(Desempleo_Col_xts)
plot(Desempleo_Col_xts)
```
Parece que sí es cíclico con una componente estacional.
...
A veces no queremos predecir, sino estudiar la estructura de autocorrelación, sus dinamicas y por eso necesitamos hacerla estacionaria.
```{r chicken1}
library(astsa)
library(TSstudio)
data(chicken)
...
$$y_t=\mu_t+a_t$$
o mas específicamente, una función lineal deterministica
...
```
De hecho "eliminamos" porque para eliminar realmente la tendencia toca hacer diferenciación. Y no es lo mismo, ya veremos.
...
```
La diferenciacion es capaz de eliminar la tendencia que tiene la serie. Vuelve la serie estacionaria, esto está garantizado, mientras que estimar y eliminar no siempre el resultado es una serie estacionaria (pero sí sin tendencia)

```{r sin tendencia chicken}
par(mar = c(2,2,2,2))
fit = lm(chicken~time(chicken), na.action=NULL) # Regresión sobre el tiempo
par(mfrow=c(2,1))
plot(resid(fit), type="l", main="sin tendencia") #y - est(y)
plot(diff(chicken), type="l", main="Primera Diferencia") #aquí hace la primera diferenciación
```
```{r acf chicken}
par(mar = c(3,2,3,2))
...
```
ACF es la función de autocorrelacion
...
```{r BoxCox}
data("AirPassengers")
plot(AirPassengers) #esto tiene heterocedasticidad, es necesario arreglar ese problema.

#####Transformación Box-Cox
library(FitAR)
library(forecast)
forecast::BoxCox.lambda(AirPassengers, method = "guerrero", lower = 0, upper = 2) ###Me entrega el valor de lambda, mete la serie, el metodo y los valores para lambda.
##method="loglik"
FitAR::BoxCox(AirPassengers)###Me entrega una gráfica
lAirPass=log(AirPassengers) #transformación logaritmica porque el lambda es muy cercano a 0.
par(mar = c(3,2,3,2))
par(mfrow=c(2,1))
...
Vamos a correr lo mismo pero en Python para la transformacion de BoxCox. Trabajar en python desde R.

Pero toca crear el environment en conda

#```{r configuración, include=FALSE}
#library(knitr) 
#library(reticulate)
#use_condaenv(condaenv = "Python38andR", conda = "auto", required = TRUE) #no debe soltar ningun error
#pandas=import("pandas")
#knitr::knit_engines$set(python = reticulate::eng_python) 
#```

Aqui arranca con python en vez de r, especifica que está en python.

#```{python inicio}
#import sys #importa donde esta el sistema
#print(sys.path)
#import pandas as pd
...
##data.set_index('Month', inplace=True) #desactivar esta monda
...
#dispersion de una serie vs los retardos de otra
par(mar = c(3,2,3,2))
...
#dev.off()
```
Vemos que en los peces hay mucha asociación en los primeros momentos. Además, parece que oscila el comportamiento.

Vamos a hacer autocorrelación cruzada, entre la presion del ambiente y peces. Importa el orden para la izquierda es:
$$
corr(rec(t), soi(t-h))
$$
Donde según la grafica el calentamiento tiene un impacto negativo en la cantidad de peces despues de al menos 6 meses. 

Pero cuando h < 0 es mejor interpretarlo de otra manera (ya que si no lo hacemos diriamos que si nacen menos peces entonces el oceano no se calienta ¿?). Pensamos mejor:
$$
corr(soi(t), rec(t-h))
$$
...
Podemos usar la función *TSstudio::ts_heatmap* para crear un mapa de calor.     El flujo de color es horizontal.
...
```{r serie desempleo en un periodo}
 unemployment <- window(USUnRate, start = c(1990,1)) #window para tomar solo una parte 
   ts_plot(unemployment,
           title = "US Monthly Unemployment Rate",
           Ytitle = "Unemployment Rate (%)",
           Xtitle = "Year",
...
* A third cycle, which began in 2007 and as of May 2019 has not been completed yet, which means that this has continued for more than 12 years
 ...
 Hay tres ciclos bien claros pero no estacionales, cómo se ve en el mapa de calor?
 Se ven verticalmente!


```{r ciclos2}
ts_heatmap(unemployment,title = "Mapa de Calor - Tasa de Desempleo EEUU")
```
...
Comportamientos que se repitan a lo largo del tiempo.
...
Porque arriba asumimos el ciclo, pero cómo lo encotramos? Existe siempre?
...
Veamos ahora con una serie de tiempo real. Usaremos la serie SOi (calentamiento del oceano pacifico) y Recruitment para ilustrar su uso. Recordemos que ambas series son mensuales, así que las frecuencias que serán dibujadas en el eje x vedrán en múltiplos de $\frac{1}{12}$. Usaremos ahora la función <b> mvspec</b> del paquete <b>astsa</b>.
